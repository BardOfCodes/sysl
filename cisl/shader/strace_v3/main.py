"""
Based on "https://www.shadertoy.com/view/3tKfDG"
// Jacquemet Matthieu
...
@inputs sun, sky
@outputs color
@dependencies LightPackage, SphereTraceV2, background, SCENE_NORMAL, SCENE_MATERIAL
@vardeps 
...
@name SphereTrace
@inputs ro, rd, max_dist
@outputs color
@dependencies SCENE_EXPRESSION
@vardeps _RAYCAST_MAX_STEPS, _ST_EPSILON
@vardeps _ADD_FLOOR_PLANE, _SCENE_RADIUS
...
// h : hit
// s : Number of steps
vec2 SphereTrace(vec3 o, vec3 u, float e,out bool h,out int s)
{
    h = false;
    ...
    float b = dot(o, o) - _SCENE_RADIUS*_SCENE_RADIUS;
    float c = dot(o, u);
    float disc = c*c - b;
    ...
    for(int i=0; i<_RAYCAST_MAX_STEPS; i++)
    {
        s=i;
        vec3 p = o+t*u;
        res = SCENE_EXPRESSION(p);
        // Hit object
        //if (abs(res.x) < 0.0001)
        if (-0.001 < res.x  && res.x < 0.0)
        //if (res.x < 0.0)
        {
            s=i;
            h = true;
...
# TBD
SphereTraceV2 = register_shader_module("""
@name SphereTraceV2
...
# SceneMaterial = register_shader_module("""
# @name SceneMaterial
# @inputs p
# @outputs color
# @dependencies 
...
# SCENE_EXPRESSION = register_shader_module("""
# @name SCENE_EXPRESSION
# @inputs p
# @outputs color
# @dependencies 
# @vardeps 
